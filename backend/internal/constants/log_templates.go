...
var LogTemplates = map[string]string{
	...
	"reservation_create_block":  "reservation create blocked, userID=%d, stationID=%d, start=%s, end=%s, reason=%s",
	"reservation_confirm_ok":    "reservation confirm success, reservationID=%d",
	"reservation_confirm_race":  "reservation confirm skipped, reservationID=%d, status=%s, reason=%s",
	...
	"reservation_checkin_block": "reservation checkin blocked, reservationID=%d, now=%s, start=%s, end=%s, reason=%s",
	"reservation_reschedule_ok": "reservation reschedule success, reservationID=%d, fromStation=%d, toStation=%d, start=%s, end=%s",
	"reservation_expire_scan":   "reservation expire scan, found=%d, cancelled=%d",
	"reservation_autocancel_ok": "reservation auto cancel success, reservationID=%d, end=%s",
